...
const Detail = ({ defectCode, defect, detail }) => {
    ...
    return( 
        <Fragment>
          ...
          /* {show && detail.map( item =><tr>
            <td className='detail_defect_machine'><div className='detail_row_production'><div></div><div>{ item.itemNumber }</div></div></td>
            {item.defect.map((it, index)=><td key={index} className='detail_defect_day'>{it}</td>)}
            </tr>
