    ...
    if (arr.length === 0) {
      ...
      return arr.map((cleaning) => {
        ...
        if (!selectedCycles) {
          ...
          const selected = selectedCycles.filter(
            (cyc) => cyc.molde === molde._id
          );
          return (
            <MoldeCalendar
              key={_id}
  ...
  return loadingCleanings ? (
    ...
    <div className="calendar-container">
      <table className="calendar-table">
        <thead>
          <tr>
            <th className="month-calendar" colSpan={getMonth().days + 1}>
              {getMonth().month}
            </th>
          </tr>
          <tr>
        ...
        {!loadingSelectedCycles &&
          renderMoldeCalendar(
            cleanings.filter(
              ...
              //     &&
              // item.date >= `${year}-${getMonth().m}-01` &&
              // item.date <= `${year}-${getMonth().m}-31`
            )
          )}
      </table>
    </div>
